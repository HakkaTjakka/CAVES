...
extern int init_shader(char* shader_name);
...
//        return-1;
    ...
    int cave_shader_index=init_shader(shader_name);
//    sf::sleep(sf::seconds(2.0));
    printf("cave_shader_index=%d\n",cave_shader_index);

    int test_shader_index=init_shader("shaders/test.frag");
//    sf::sleep(sf::seconds(2.0));
    printf("test_shader_index=%d\n",test_shader_index);
    ...
    texture_ff[0].create(main_texture_pointer->getSize().x*5,main_texture_pointer->getSize().y*5);
    texture_ff[1].create(main_texture_pointer->getSize().x*5,main_texture_pointer->getSize().y*5);
    sprite_ff[0].setTexture(texture_ff[0].getTexture(),true);
    sprite_ff[1].setTexture(texture_ff[1].getTexture(),true);
    sprite_ff[0].setOrigin( texture_ff[0].getTexture().getSize().x/2.0, texture_ff[0].getTexture().getSize().y/2.0 );
    sprite_ff[1].setOrigin( texture_ff[1].getTexture().getSize().x/2.0, texture_ff[1].getTexture().getSize().y/2.0 );
    sprite_ff[0].setRotation(1.0);
    sprite_ff[1].setRotation(1.0);
    sprite_ff[0].setPosition(texture_ff[0].getTexture().getSize().x/2.0, texture_ff[0].getTexture().getSize().y/2.0);
    sprite_ff[1].setPosition(texture_ff[1].getTexture().getSize().x/2.0, texture_ff[1].getTexture().getSize().y/2.0);
...
//    texture_ff[1].draw(main_sprite,shader_struct_vector[test_shader_index]->RenderStates);
    texture_ff[1].display();

    int flip_flop=0;
//    app.draw(sprite_ff[0]);
//    app.display();
    ...
    shader_struct_vector[cave_shader_index]->Shader.setUniform("chunk_offsetx", chunk_offsetx);
    shader_struct_vector[cave_shader_index]->Shader.setUniform("chunk_offsetz", chunk_offsetz);

    sf::BlendMode MyBlendingMode(   sf::BlendMode::One,
                                    sf::BlendMode::One,
                                    sf::BlendMode::Add,
                                    sf::BlendMode::One,
...
//    sf::BlendMode BlendingMode_backup=shader_struct_vector[cave_shader_index]->RenderStates.blendMode;
    ...
    for (int y = 0; y < 256; y++) {
        update_caves_region(texture_y, main_sprite, y, cave_shader_index);
    }
...
//        app.draw(sprite_y[y],MyBlendingMode);
        app.draw(main_sprite,MyBlendingMode);
        app.display();
    }
    printf("Textures displayed\n");
...
//    float angle=0.0;
    ...
    */
    ...
    if (gpu) {
        sf::Clock m_clock;
        shader_struct_vector[cave_shader_index]->Shader.setUniform("phase", int(1));
    ...
    //    shader_struct_vector[cave_shader_index]->RenderStates.blendMode=MyBlendingMode;
    //    shader_struct_vector[cave_shader_index]->RenderStates.blendMode=sf::BlendNone;
    //    shader_struct_vector[cave_shader_index]->RenderStates.blendMode=sf::BlendAlpha;
    //    shader_struct_vector[cave_shader_index]->RenderStates.blendMode=0;
    //    shader_struct_vector[cave_shader_index]->RenderStates.blendMode=BlendingMode_backup;
    //    shader_struct_vector[cave_shader_index]->RenderStates.blendMode=BlendingMode_backup;
    ...
    //        renderstate.blendMode=MyBlendingMode;
            ...
            for (int x = 0; x < 512; x++) {
                toggle2();
                shader_struct_vector[cave_shader_index]->Shader.setUniform("chunk_offsetx", x);
                for (int z = 0; z < 512; z++) {
                    ...
                    if (pixel!=0) {
                        shader_struct_vector[cave_shader_index]->Shader.setUniform("chunk_offsetz", z);
    ...
    //                    main_sprite.setPosition(0,0);
                        ...
                        for (int yyy=-17; yyy<18; yyy++) {
                            if (yyy+y>=0 && yyy+y<256) {
                                shader_struct_vector[cave_shader_index]->Shader.setUniform("height", yyy);

                                if (yyy!=17) texture_fill[y+yyy].display();

                                main_sprite.setTexture(texture_fill[y+yyy].getTexture());

                                texture_y[y+yyy].draw(main_sprite,shader_struct_vector[cave_shader_index]->RenderStates);
                                draws++;
                            }
                        }
                        ...
                        for (int yyy=-17; yyy<18; yyy++) {
                            if (yyy+y>=0 && yyy+y<256) {
                                ...
                                texture_fill[y+yyy].draw(main_sprite);
                            }
                        }
            ...
            printf("%3d match_plus=%8d no_match=%8d id_count=%8d pixel_count=%8d draws=%8d\n",y,match_plus,no_match,id_count,pixel_count,draws);
            ...
            for (int yyy=-17; yyy<18; yyy++) {
                if (yyy+y>=0 && yyy+y<256) {
                    ...
                    texture_fill[y+yyy].display();
                }
            }
            app.draw(sprite_app[0]);
            app.draw(sprite_app[1]);
            app.draw(sprite_app[2]);
...
//        while (repl_str(cmd,"/","\\"));
    ...
    delete_shader(test_shader_index);
//    main_mutex.lock();
//    main_texture_pointer=backup_main_texture_pointer;
//    main_sprite_pointer=backup_main_sprite_pointer;
//    main_mutex.unlock();
...
void update_caves_region(sf::RenderTexture *texture, sf::Sprite& sprite, int texture_index, int shader_index) {
//    shader_struct_vector[shader_index]->Shader.setUniform("height", float(texture_index));
//    printf("\nhier1\n");
    shader_struct_vector[shader_index]->Shader.setUniform("height", texture_index);
//    printf("\nhier2\n");
    texture[texture_index].draw(sprite,shader_struct_vector[shader_index]->RenderStates);
//    printf("\nhier3\n");
//    texture[texture_index].draw(sprite);
}
