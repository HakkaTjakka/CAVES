...
unsigned char* region_block;
                ...
                if (main_CAVES_simple(region_x, region_z, region_block, shader_name)==-1) return;
...
//    srand(time(0));
    ...
    int cave_shader_index=init_shader(shader_name);         printf("cave_shader_index=%d\n",cave_shader_index);
...
//                                    if ((!(rand()%500)) && AY[y].id==95) AY[y]=BlockInfo(89,0,0,0); //glowstone
    ...
    //        renderstate.blendMode=MyBlendingMode;
            ...
            for (int x = 0; x < 512; x++) {
                toggle2();
                shader_struct_vector[cave_shader_index]->Shader.setUniform("offsetx", x);
                for (int z = 0; z < 512; z++) {
                    ...
                    if (pixel!=0) {
                        shader_struct_vector[cave_shader_index]->Shader.setUniform("offsetz", z);
