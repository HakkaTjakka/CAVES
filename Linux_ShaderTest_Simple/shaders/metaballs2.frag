...
//    int c=get_color(int(st.x*512.0),int(st.y*512.0),int(u_time));
    ...
    for (int me=0; me<3; me++) {
        float m=sin(sin(float(me)*u_time+st.x+st.y/2.0));
        float m1=sin(sin(float(me)*u_time*1.3)+st.x/2.0+st.y);
        float m2=sin(float(me)*u_time*1.4);
        vec2 t=vec2(sin(u_time+m)+0.4*sin(u_time*2.4+m2),
                    sin(u_time*.9+m)+0.4*sin(u_time*2.5-m));
        ...
        for (int j= -1; j <= 1; j++ ) {
            for (int i= -1; i <= 1; i++ ) {
                ...
                // Animate the offset
                offset = vec2(0.5 + 0.5*sin(u_time + 6.2831*offset.x+m),
                             0.5 + 0.5*sin(u_time*0.84 + 6.1831*offset.y+m2));
	    ...
	    color += step(0.260+0.10*sin(m1+m2+m), m_dist);
    }
...
void main() {
    float u_time=float(height)/150.0;
    vec2 st = ( offset + gl_TexCoord[0].xy )/3.0;
