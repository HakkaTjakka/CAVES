...
//    mcglobal=11;
//    launch_MCEDITOR();
//    while (MCEDITOR_running==0) {
//        sleep(100);
//    }
//    while (MCEDITOR_running==1) {
//        sleep(1000);
//    }
//}
...
//unsigned char region_block2[512*256*512*4];

void make_caves() {
//    printf("unsigned char region_block[512*256*512*4];\n");
//    sf::sleep(sf::seconds(1.0));
//    printf("std::memset(region_block, 0x0, 512*256*512*4);\n");
//    sf::sleep(sf::seconds(1.0));
//    std::memset(region_block2, 0x0, 512*256*512*4);
    ...
    while (size_regions<60) {
        ...
        for (region_x=0; region_x<size_regions && MCEDITOR_stop==0; region_x++) {
            for (region_z=0; region_z<size_regions && MCEDITOR_stop==0; region_z++) {
                ...
                printf("CREATING CAVES: X=%3d Z=%3d\n",region_x,region_z);
...
//2
                            for (int n=0; n<4; n++) {
                                ...
                                r2= 255+int( 230.0*sin( (
                                           0.7*( var2*f_x/2.23  +  f_z/2.3 )                       /   (97.3 * (1.0+var2*0.4*sin((-var3*f_x/1.2 + f_y     + f_z/1.5) / 133.0)))   +
                                           var2*0.6*( var3*f_x/1.44  +  f_z/1.5 )                            /   (113.3 * (1.0+var3*0.4*sin((-f_x-      var2*f_y/1.6 + f_z/1.3) / 124.0)))   +
                                           var3*1.8*sin( (var3*f_x/1.53 + f_z/1.1 )                /   (86.0 * (1.0+0.4*sin(( f_x + f_y     - var1*f_z/2.1    ) / 83.0)))   ) +
                                           var1*2.15*sin( (f_x/1.33 + var2*f_z/1.4 )                /   (98.0 * (1.0+0.4*sin(( f_x/1.8 - var3*f_y     + f_z/1.4) / 87.0)))   )
                                    )*0.6  ) );
                                r1= 255+int( 230.0*sin( (
                                       0.7*( var3*f_z/2.02   +  f_x/2.30 )              / (126.3*(1.0+var3*0.3*sin(( f_x/1.2 + f_y/2.2 + var2*f_z      )  /117.0)))    +
                                       var3*0.6*( f_z/1.12   +  var1*f_x/1.22 )                  / (118.3*(1.0+var1*0.4*sin((-var3*f_x/1.3 - f_y     + f_z/1.7  )  /125.0)))    +
                                ...
                                if (n==0) {
                                        r3_x   = r3   + height_add;   r1_x   = r1   ;    r2_x   = r2   ;
                                        r3_x_b = r3_b + height_add;   r1_x_b = r1_b ;    r2_x_b = r2_b ;
                                } else if (n==1)  {
                                        r3_y   = r3   + height_add;   r1_y   = r1   ;    r2_y   = r2   ;
                                        r3_y_b = r3_b + height_add;   r1_y_b = r1_b ;    r2_y_b = r2_b ;
                                } else if (n==2)  {
                                        r3_z   = r3   + height_add;   r1_z   = r1   ;    r2_z   = r2   ;
                                        r3_z_b = r3_b + height_add;   r1_z_b = r1_b ;    r2_z_b = r2_b ;
                                } else  {
                                        r3     = r3   + height_add;   r1     = r1   ;    r2     = r2   ;
                                        r3_b   = r3_b + height_add;   r1_b   = r1_b ;    r2_b   = r2_b ;
                                }
...
*/
        ...
        if (!plotting) printf("\n");
        printf("\r%8d added. %8d replaced. Total = %8d = %5.2f%%  tears = %d  water=%d tnt=%d\n",
            num_blocks_added, num_blocks_replaced, num_blocks_total, 100.0*(float)num_blocks_total/(512*512*256),tears,water,tnt_blocks);
    }
...
//    }
    ...
    for (int x = 0; x < xl; x++) {
        ...
        for (int z = 0; z < zl; z++) {
            ...
            for (int y = 0; y < 256; y++) {
                BlockInfo bi=AY[y];
                if (bi.id==46 ||  bi.id==89 ||  bi.id==95 || bi.id==5) {
//                    if (bi.id!=0) {
//                        if (min_y==-1 && bi.id==251) min_y=y;
                    if (min_y==-1) min_y=y;
                    num_blocks++;
